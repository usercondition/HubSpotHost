...
/** A configured callback token is an optional signature fallback for private-app
 * deliveries. Without an explicitly configured hash, this route fails closed. */
export function verifyCallbackToken(
  token: string | undefined,
  expectedHash: string | undefined = process.env.HUBSPOT_CALLBACK_TOKEN_SHA256?.trim(),
): boolean {
