        ...
        {!isUnlocked ? (
          ...
          <>
            ...
            >
              ...
              <div className="overflow-x-auto bg-muted/25 p-3 md:p-4">
                <div className="flex min-h-[28rem] min-w-max items-stretch gap-3">
                  {columns.map((column) => (
                    <div
                      key={column.id}
                      className="flex w-[17rem] shrink-0 flex-col rounded-lg border border-border/80 bg-background/90"
                      data-testid={`column-deal-stage-${column.id}`}
                    >
                      <div
                        className={cn(
                          "rounded-t-lg border-b border-border px-3 py-2.5",
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <span className="numeric shrink-0 rounded-md bg-background/80 px-1.5 py-0.5 text-[0.6875rem] font-medium text-muted-foreground">
                            {column.deals.length}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-1 flex-col gap-2 p-2">
                        {column.deals.length === 0 ? (
                          <div className="flex flex-1 items-center justify-center rounded-md border border-dashed border-border/70 px-2 py-10">
                      ...
                      <div className="mt-auto space-y-1 rounded-b-lg border-t border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
...
function SummaryStat({
  ...
  return (
    <div className="rounded-lg border border-card-border bg-card px-4 py-3" data-testid={testId}>
      <p className="rule-label">{label}</p>
      <p className={cn("mt-1 text-xl font-semibold tracking-tight numeric", warn && "text-primary")}>{value}</p>
...
function DealCard({
  ...
  return (
    <article
      className="group rounded-md border border-border bg-card p-3 shadow-sm transition-colors hover:border-primary/35"
    ...
    >
      ...
      <dl className="mt-2.5 space-y-1.5 text-xs text-muted-foreground">
        ...
        <div className="flex items-center gap-2">
          ...
          <dd className="font-medium text-foreground numeric">{money(deal.amount)}</dd>
        </div>
        {deal.contactName ? (
          <div className="flex items-center gap-2">
