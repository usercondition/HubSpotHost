...
test("performance summarizes recent deals and ranks low margins before incomplete costs", () => {
  ...
  assert.equal(snapshot.activeDeals[0]?.amount, 350);
  assert.equal(snapshot.activeDeals[0]?.productionCost, 260);
  assert.equal(snapshot.activeDeals[0]?.grossProfit, 90);
  assert.equal(snapshot.activeDeals[0]?.marginPercentage, 25.71);
  assert.equal(snapshot.activeDeals[0]?.costsComplete, true);
  assert.equal(snapshot.activeDeals[1]?.amount, 100);
  assert.equal(snapshot.activeDeals[1]?.productionCost, 0);
  assert.equal(snapshot.activeDeals[1]?.grossProfit, 100);
  assert.equal(snapshot.activeDeals[1]?.costsComplete, false);
  ...
  assert.equal(snapshot.closedDeals[0]?.amount, 50);
  assert.equal(snapshot.closedDeals[0]?.productionCost, 30);
  assert.equal(snapshot.closedDeals[0]?.grossProfit, 20);
  assert.equal(snapshot.closedDeals[0]?.marginPercentage, 40);
  assert.equal(snapshot.closedDeals[0]?.costsComplete, true);
